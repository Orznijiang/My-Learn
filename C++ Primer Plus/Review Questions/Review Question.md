...
* 重载`+`、`-`等二元运算符时，返回值设为引用可能会导致返回被销毁的临时变量从而出错
...
* 继承时赋值运算符需要重新定义，而类型不同的加法（如加个int）在需求没有改变的情况下不需要重新定义
* 赋值运算符的重载不能是虚函数，因为赋值运算符的参数即是当前类类型，不能随着指针类型变化，输出运算符等同理
...
1. **使用大括号括起的初始化列表语法重写下属代码。重写后的代码不应使用数组`ar`：**
...
2. **在下述简短的程序中，哪些函数调用不对？为什么？对于合法的函数调用，指出其引用参数指向的是什么。**
   ...
   2. 合法。引用参数指向一个被初始化为`w + 1`的临时分量

   3. 合法。引用参数指向一个被初始化为`up(w)`的返回值的临时分量

      一般而言，将左值传递给`const`左值引用参数时，参数将被初始化为左值。将右值传递给参数时，`const`左值引用参数将指向右值的临时拷贝
   ...
   8. 合法。引用参数指向`w + 1`的临时拷贝

   9. 合法。引用参数指向`up(w)`的返回值的临时分量
...
5. **假设`Fizzle`类只有如下所示的数据成员：**
   ...
   **为什么不适合给这个类定义移动构造函数？要让这个类适合定义移动构造函数，应如何修改存储4000个`double`值的方式？**

   在转让数据所有权（而不是复制数据）可行时，可使用移动构造函数，但对于标准数组，没有转让其所有权的机制。如果`Fizzle`使用指针和动态内存分配，则可将数据的地址赋给新指针，以转让其所有权

6. **修改下述简短的程序，使其使用Lambda表达式而不是`f1()`。请不要修改`show2()`。**
...
7. **修改下述简短而丑陋的程序，使其使用Lambda表达式而不是函数符`Adder`。请不要修改`sum()`。**
...
* 不能在函数内部定义其他函数，但能在函数内部定义类（包含函数符类）和Lambda

* 在C++中引入Lambda的主要目的是将类似于函数的表达式用作接受函数指针或函数符的函数的参数

* **包装器：18.5**

* C++11提供了一个用省略号表示的元运算符（meta-operator），让你能够声明表示模板参数包的标识符，模板参数包基本上是一个类型列表。同样，它还让你能够声明表示函数参数包的标识符，而函数参数包基本上是一个值列表
