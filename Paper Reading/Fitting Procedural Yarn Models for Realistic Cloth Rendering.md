...
我们正在发布他们的实现、测量数据和拟合参数（表 2）。 他们的开发的工具和数据允许将纤维级细节添加到基于纱线的布料设计中，这种布料设计在纺织品软件<sup>[12]</sup>和模拟工具<sup>[1] [7]</sup>中被使用。 添加的细节可以显着提高渲染质量（图 1 和 图 17），对设计应用程序来说作用巨大。
...
此外，该模型有一个单独的步骤，我们在 §5 中对其进行了改进，用于处理对于真实感来说非常重要的飞散纤维的效果。 在施罗德等人的研究<sup>[14]</sup>中可了解更多详情。
